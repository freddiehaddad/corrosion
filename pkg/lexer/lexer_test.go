...
func compareToken(t *testing.T, i int, et expectedToken, nt token.Token) {
	if nt.Type != et.expectedType {
		t.Errorf("tests[%d] - token type wrong: expected=%q got=%q", i, et.expectedType, nt.Type)
	}
	if nt.Literal != et.expectedLiteral {
		t.Errorf("tests[%d] - token literal wrong: expected=%q got=%q", i, et.expectedLiteral, nt.Literal)
	}
}
